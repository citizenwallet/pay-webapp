"use client";

import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar";
import { Card, CardContent, CardHeader } from "@/components/ui/card";
import { getAvatarUrl } from "@/lib/utils";
import { useOrders } from "@/state/orders/actions";
import { selectSelectedOrder } from "@/state/orders/selectors";
import { useOrdersStore } from "@/state/orders/state";
import { getBaseUrl } from "@/utils/deeplink";
import { Flex, Text } from "@radix-ui/themes";
import { ArrowLeft, Calendar, DollarSign, FileText, Package } from "lucide-react";
import Link from "next/link";
import { useRouter } from "next/navigation";
import { useEffect, useMemo } from "react";

interface OrderDetailPageProps {
    orderId: string;
}

export default function OrderDetailPage({ orderId }: OrderDetailPageProps) {
    const selectedOrder = useOrdersStore(selectSelectedOrder);
    const [, ordersActions] = useOrders(getBaseUrl());
    const router = useRouter();

    const formattedDate = useMemo(() => {
        if (!selectedOrder) return "";
        return new Date(selectedOrder.created_at).toLocaleDateString('en-US', {
            weekday: 'long',
            year: 'numeric',
            month: 'long',
            day: 'numeric',
            hour: '2-digit',
            minute: '2-digit'
        });
    }, [selectedOrder]);


    useEffect(() => {
        if (!selectedOrder && orderId) {
            ordersActions.setSelectedOrderById(orderId);
        }
    }, [selectedOrder, orderId, ordersActions]);


    const goBack = () => {
        router.back();
    }

    if (!selectedOrder) {
        return (
            <div className="relative flex min-h-screen w-full flex-col align-center p-4 max-w-xl">
                <div className="mb-6">
                    <Link
                        href={'/'}
                        className="inline-flex items-center text-sm text-gray-600 hover:text-gray-900 transition-colors"
                    >
                        <ArrowLeft className="h-4 w-4 mr-2" />
                        Back to Orders
                    </Link>
                </div>
                <Card className="shadow-lg border-0">
                    <CardContent className="p-8 text-center">
                        <Text size="4" className="text-gray-600">
                            Order not found. Please go back and select an order from the list.
                        </Text>
                    </CardContent>
                </Card>
            </div>
        );
    }

    const formatAddress = (address: string) => {
        return `${address.slice(0, 6)}...${address.slice(-4)}`;
    };

    const getItemNameById = (itemId: number) => {
        if (!selectedOrder.place?.items) return `Item ${itemId}`;

        const placeItem = selectedOrder.place.items.find(item => item.id === itemId);
        return placeItem?.name || `Item ${itemId}`;
    };


    const getItemPriceById = (itemId: number) => {
        if (!selectedOrder.place?.items) return `Item ${itemId}`;

        const placeItem = selectedOrder.place.items.find(item => item.id === itemId);
        return placeItem?.price;
    };


    return (
        <div className="relative flex min-h-screen w-full flex-col align-center p-4 max-w-xl">
            <div className="mb-6">
                <div
                    onClick={goBack}
                    className="inline-flex items-center text-sm text-gray-600 hover:text-gray-900 transition-colors"
                >
                    <ArrowLeft className="h-4 w-4 mr-2" />
                    Back to Orders
                </div>
            </div>

            <Card className="shadow-lg border-0">
                <CardHeader className="pb-4">
                    <Flex direction="column" gap="4">
                        <Text size="6" weight="bold" className="text-gray-900">
                            Order #{selectedOrder.id}
                        </Text>

                        <Flex align="center" gap="4" className="p-4 bg-gray-50 rounded-lg">
                            <Avatar className="h-16 w-16 border-2 border-primary">
                                <AvatarImage
                                    src={getAvatarUrl(selectedOrder.place?.display, selectedOrder.account)}
                                    alt={`${selectedOrder.place?.slug || 'Place'} profile`}
                                    className="object-cover"
                                />
                                <AvatarFallback className="text-lg font-semibold">
                                    {selectedOrder.place?.slug?.charAt(0).toUpperCase() || "P"}
                                </AvatarFallback>
                            </Avatar>
                            <Flex direction="column" gap="1">
                                <Text size="5" weight="bold" className="text-gray-900">
                                    {selectedOrder.place?.slug}
                                </Text>
                                <Text size="3" className="text-gray-600">
                                    {formatAddress(selectedOrder.account)}
                                </Text>
                            </Flex>
                        </Flex>
                    </Flex>
                </CardHeader>

                <CardContent className="space-y-6">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="flex items-center gap-3 p-4 bg-blue-50 rounded-lg">
                            <div className="p-2 bg-blue-100 rounded-full">
                                <DollarSign className="h-5 w-5 text-blue-600" />
                            </div>
                            <div>
                                <Text size="2" className="text-gray-600 block">Amount</Text>
                                <Text size="5" weight="bold" className="text-blue-600">
                                    {(selectedOrder.total / 100).toFixed(2)}
                                </Text>
                            </div>
                        </div>

                        <div className="flex items-center gap-3 p-4 bg-green-50 rounded-lg">
                            <div className="p-2 bg-green-100 rounded-full">
                                <Package className="h-5 w-5 text-green-600" />
                            </div>
                            <div>
                                <Text size="2" className="text-gray-600 block">Status</Text>
                                <div className="mt-1">
                                    {selectedOrder.status}
                                </div>
                            </div>
                        </div>
                    </div>

                    <div className="flex items-center gap-3 p-4 bg-purple-50 rounded-lg">
                        <div className="p-2 bg-purple-100 rounded-full">
                            <Calendar className="h-5 w-5 text-purple-600" />
                        </div>
                        <div>
                            <Text size="2" className="text-gray-600 block">Order Date</Text>
                            <Text size="4" weight="medium" className="text-purple-600">
                                {formattedDate}
                            </Text>
                        </div>
                    </div>

                    {selectedOrder.items && selectedOrder.items.length > 0 && (
                        <div className="space-y-3">
                            <div className="flex items-center gap-2">
                                <Package className="h-5 w-5 text-gray-600" />
                                <Text size="4" weight="bold" className="text-gray-900">
                                    Items ({selectedOrder.items.length})
                                </Text>
                            </div>
                            <div className="bg-gray-50 rounded-lg p-4 space-y-3">
                                {selectedOrder.items.map((orderItem, index) => (
                                    <div key={orderItem.id || index} className="flex justify-between items-center">
                                        <div className="flex-1">
                                            <Text size="3" weight="medium" className="text-gray-900">
                                                {getItemNameById(orderItem.id)}
                                            </Text>
                                            <Text size="2" className="text-gray-600 ml-4">
                                                Qty: {orderItem.quantity || 1}
                                            </Text>
                                        </div>
                                        <Text size="3" weight="bold" className="text-blue-600">
                                            {((Number(getItemPriceById(orderItem.id) ?? 0) * (orderItem.quantity ?? 1)) / 100).toFixed(2)}
                                        </Text>
                                    </div>
                                ))}

                                <div className="border-t pt-3 mt-3">
                                    <div className="flex justify-between items-center">
                                        <Text size="4" weight="bold" className="text-gray-900">
                                            Total
                                        </Text>
                                        <Text size="5" weight="bold" className="text-blue-600">
                                            {(selectedOrder.total / 100).toFixed(2)}
                                        </Text>
                                    </div>
                                </div>
                            </div>
                        </div>
                    )}

                    {selectedOrder.description && selectedOrder.description.trim() !== "" && (
                        <div className="space-y-3">
                            <div className="flex items-center gap-2">
                                <FileText className="h-5 w-5 text-gray-600" />
                                <Text size="4" weight="bold" className="text-gray-900">
                                    Description
                                </Text>
                            </div>
                            <div className="bg-gray-50 rounded-lg p-4">
                                <Text size="3" className="text-gray-700 leading-relaxed">
                                    {selectedOrder.description}
                                </Text>
                            </div>
                        </div>
                    )}

                </CardContent>
            </Card>
        </div>

    );
}